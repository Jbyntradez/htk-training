...
const stack = [
  "Performance module library",
  "Exercise explanations and coaching cues",
  "Beginner, intermediate, and advanced variations",
  "Video demonstration placeholders",
  "Progress dashboard",
  "Coach-ready safety and progression notes"
];
      ...
      <main>
        <section className="border-b border-white/10 py-20 md:py-28">
          <div className="container-px mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1fr_0.75fr] lg:items-start">
            <div>
              <p className="text-sm font-black uppercase text-accent/45">The HTK platform</p>
              <h1 className="mt-4 text-5xl font-black leading-[0.95] sm:text-7xl">
                Train for performance that carries over.
              </h1>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-accent/65">
                A structured training library for explosiveness, endurance, mobility, athleticism, functional
            ...
            <div className="mt-10 grid gap-4 md:grid-cols-2">
              {modules.map((module) => (
                <article key={module.id} className="rounded-md border border-white/10 bg-primary p-6">
                  <p className="text-xs font-black uppercase text-accent/45">
                    {module.difficultyLevel} | {module.exercises.length} exercises
                  </p>
                  <h3 className="mt-3 text-2xl font-black">{module.title}</h3>
                  <p className="mt-3 text-sm leading-6 text-accent/60">{module.shortDescription}</p>
                </article>
              ))}
            </div>
          </div>
